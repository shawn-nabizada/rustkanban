...
pub async fn callback(
    ...
    let github_token = match exchange_code_for_token(&state.config, &params.code).await {
        Ok(token) => token,
        Err(e) => {
    ...
    let gh_user = match fetch_github_user(&github_token).await {
        Ok(user) => user,
        Err(e) => {
    ...
    // Generate bearer token and store hash
    ...
    // Redirect to CLI or show token in browser
    if let Some(port) = pending.redirect_port {
        ...
        // Headless mode: render token in the browser
        Html(format!(
            r#"<!DOCTYPE html>
<html>
<head><title>RustKanban Auth</title></head>
<body style="font-family: monospace; padding: 2em;">
<h2>Authentication successful!</h2>
<p>Copy the following token into your CLI:</p>
<pre style="background: #f4f4f4; padding: 1em; border-radius: 4px;">{raw_token}</pre>
...
async fn exchange_code_for_token(config: &Config, code: &str) -> Result<String, String> {
